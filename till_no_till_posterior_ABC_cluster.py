...
def SEIRB_network_tillage(
    ...
    B0_till=4000.0       # initial B for tilled plots 
):
...
# Define the distance that we will use, the standardized Euclidean distance or Absolute distance
...
def run_grid_search(initial_infectednodes, label="scenario"):
    """
    Fast(er) grid search using LOG-SPACED grids for positive params (no zeros),
    thinner levels, same return signature as original function.
    """
    fixed_args_model = {
        "timesteps": 139,             # shorter horizon just for grid (speeds up)
        "sigma": 1/10,
        "gamma": 1/75,
        "r": 0.001,
        "k": 60000,
    ...
    # remove 0; use small # of points. Adjust if need a bit more coverage.
    theta_range    = np.logspace(-6, -3, 5)      # 5 pts: 1e-6..1e-3
    beta_non_range = np.logspace(-9, -5, 5)     # 5 pts: 1e-9..1e-5
...
def get_initial_seeds(count=52, cluster_center=None):
    if cluster_center is None:
        cluster_center = [1000, 300]  # change this place

    nn = NearestNeighbors(n_neighbors=count)
    nn.fit(flat_coordinates)
...
for scenario in scenarios:
    ...
    seeds = get_initial_seeds(cluster_center = [1000, 300]) # always need to change
    ...
    file_name = f"posterior_samples_{scenario}_August_8_28.txt"
